...
#Region "1. Variables Globales y Propiedades"
    ...
    ' los combos los relleno yo a mano
...
#Region "2. Eventos de Inicialización (Load)"
    Private Sub FrmAlbaranesCompra_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        ' boton de imprimir, en realidad saca un pdf
...
#Region "3. Configuración UI y Diseño"
    ...
    Private Sub ConfigurarColumnasGrid()
        ...
        ' OJO: aqui es CantidadRecibida, la que entra de verdad al almacen
...
#Region "4. Lógica de Negocio y Cálculos"
    ...
    Private Sub ConfigurarEstructuraDataTable()
        With _dtLineas.Columns
            ...
            ' columnas del iva y las que calculo yo
    ...
    Private Function GenerarProximoNumeroAlbaran() As String
        ' tiro de NumeradorDocumentos. si lo hago a pelo me ordena mal (ALC-10 antes que ALC-2)
...
#Region "5. Persistencia (Base de Datos - CRUD)"
    Private Sub CargarAlbaran(numeroAlbaran As String)
        Try
            ...
            Using cmd As New SQLiteCommand(sqlLin, c)
                ...
                ' recalculo el iva y los totales aqui mismo
                For Each row As DataRow In _dtLineas.Rows
    ...
    Private Sub GuardarAlbaran()
        ...
        ' la columna NumeroAlbaranProveedor es NOT NULL, si el user no la rellena pongo la mia y listo
        ...
        ' id del pedido de donde sale, puede no haber
        ...
        Try
            ...
            ' sumo todo para meterlo en la cabecera
            ...
            ' si estoy editando, pillo el id interno otra vez
            If Not esNuevo Then
            ...
            ' fecha que usare en los movimientos de almacen
            Dim fechaAlb As DateTime
            If Not DateTime.TryParse(TextBoxFecha.Text, fechaAlb) Then fechaAlb = DateTime.Now

            ' 2. borrar lineas que se han quitado. ojo: si quito una linea hay que devolver el stock
            ...
            ' truco: si el albaran viene de un pedido, le pongo al pedido el estado "Recibido"
            ' me lo dijo el profe, asi no hay que hacerlo a mano cada vez
    ...
    ' me pasa de PEC-XXX al id interno del pedido
    ...
    Private Sub ImportarDatosPedido(numeroPedido As String)
        ...
        Try
            ...
            Using cmd As New SQLiteCommand(sqlCab, c)
                ...
                Using r = cmd.ExecuteReader()
                    If r.Read() Then
                        ...
                        ' el comprador, dejo el mismo del pedido si lo tenia
                        TextBoxIdComprador.Text = If(IsDBNull(r("ID_Comprador")), "", r("ID_Comprador").ToString())
            ...
            For Each rowOrig As DataRow In dtOrigen.Rows
                ...
                ' por defecto pongo lo mismo que pediste, ya lo cambia el user si llega menos
